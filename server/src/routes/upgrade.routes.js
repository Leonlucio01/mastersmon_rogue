import { Router } from 'express'
import { getRequestCharacter } from '../services/gameData.js'
import {
  getUpgradeState,
  upgradeEquipment,
} from '../services/upgrades.js'

const router = Router()

async function getCharacter(request, response) {
  const character = await getRequestCharacter(request.user?.id)
  if (!character) {
    response.status(404).json({ error: 'No existe un personaje activo.' })
    return null
  }
  return character
}

router.get('/', async (request, response, next) => {
  try {
    const character = await getCharacter(request, response)
    if (!character) return
    response.json(await getUpgradeState(character.id))
  } catch (error) {
    next(error)
  }
})

router.post('/equipment', async (request, response, next) => {
  try {
    const character = await getCharacter(request, response)
    if (!character) return
    if (!request.body.inventoryItemId) {
      return response
        .status(400)
        .json({ error: 'Selecciona un equipo para mejorar.' })
    }
    response.json(
      await upgradeEquipment(character.id, request.body.inventoryItemId),
    )
  } catch (error) {
    next(error)
  }
})

export default router
